  ...
  const options = {
    renderNode: {
      ...
      [BLOCKS.HEADING_3]: (_node: any, children: React.ReactNode) => (
        <h3 className="font-display font-extrabold text-2xl text-charcoal mb-4 mt-8">
          {children}
        </h3>
      ),
      [BLOCKS.HEADING_4]: (_node: any, children: React.ReactNode) => (
        <h4 className="font-display font-extrabold text-xl text-charcoal mb-3 mt-6">
          {children}
        </h4>
      ),
      [BLOCKS.UL_LIST]: (_node: any, children: React.ReactNode) => (
        <ul className="list-disc list-outside space-y-3 mb-6 text-lg text-neutral-600 pl-6">
          {children}
        </ul>
      ),
      [BLOCKS.OL_LIST]: (_node: any, children: React.ReactNode) => (
        <ol className="list-decimal list-outside space-y-4 mb-6 text-lg text-neutral-600 pl-6">
          {children}
        </ol>
      ),
      ...
      [INLINES.HYPERLINK]: (node: any, children: React.ReactNode) => (
        <a
          href={node.data.uri}
          className="text-fluent-red hover:text-charcoal underline"
